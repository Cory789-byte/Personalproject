# ⭐⭐⭐ IS THE MEDICAL COMPELLING AND CONTEMPORANEOUS, AS PUT?
**10 September 2026 · assessment of the Schedule of Medical Documents served 9 September 2026**

> ⭐⭐⭐ **VERDICT: outstanding on contemporaneity. Compelling on causation — CONDITIONALLY. Two
> questions must be answered before either doctor enters the box, and both answers live in the one
> item he does not hold.**

---

# 1. ⭐⭐⭐ CONTEMPORANEITY — THIS IS THE STRONGEST FEATURE AND IT IS NOT CLOSE

⭐⭐⭐ **NOT ONE DOCUMENT IN THE SET WAS MADE FOR THIS PROCEEDING.** The schedule says so in terms:
*"No report has been prepared for the purposes of this proceeding."*

| | Created | ⭐ Why it cannot be attacked as forensic |
|---|---|---|
| **GP record 1 Jan 2023 – 1 Jul 2024** | before onset and before any claim | ⭐⭐⭐ **obtained by the RESPONDENT under its OWN Form 29 of 4 July 2025, held at its item 11** |
| **WCCs 1 Jul – 8 Sep 2024** | at the time of certification | statutory forms, medication box unticked |
| **Practice email 24 Oct 2024, 11:45 am** | ⭐⭐ **the morning of the first consultation** | contemporaneous written confirmation |
| **Report 13 Feb 2025** | ⭐⭐⭐ **prepared for QSuper income protection** | ⭐⭐⭐ **and marked "not for medico-legal use" — no forensic purpose is possible** |
| **ECC 3 Jul 2026** | employment process | effect and capacity only |

⇒ ⭐⭐⭐ **The whole spine is pre-litigation, and the two most valuable items came out of the
Respondent's own disclosure.**

## 1.1 ⭐⭐⭐⭐ THE BEST ITEM IN THE SET — 16 NOVEMBER 2023
> Dr Nanayakkara records **poor sleep with shift work**, that the Appellant **could not do shifts
> without a good sleep**, ⭐⭐⭐ **no psychological illness such as depression or psychosis, mood good**,
> melatonin and temazepam prescribed.

⇒ ⭐⭐⭐ **It does BOTH jobs at once, seven months before onset, from their own item 11: it closes the
pre-existing-illness attack AND it opens the fatigue thread — with a prescription — before any claim
existed.** ⭐⭐ **Nothing an IME writes in 2026 competes with that.**

## 1.2 ⭐⭐⭐⭐ THE SHARPEST JUXTAPOSITION — 15 → 16 MAY 2024
⭐ **15 May 2024, 6:23 pm:** Ms Reese emails asking him to retract his email (**facts 76 and 77,
admitted**).
⭐⭐⭐ **16 May 2024:** Dr Zhao renews the referral to a psychiatrist *"for ongoing care and
management"*.
⇒ ⭐⭐⭐⭐ **An admitted workplace event, and a psychiatric referral the next day. It is not argued in
the schedule — it is simply placed, with the admitted fact numbers beside it. That is the correct
way to do it and it is devastating.**

## 1.3 ⭐⭐⭐ AND THE CAUSATION ACCOUNT WAS GIVEN TO A GP BEFORE ANY CLAIM DECISION
⭐ **28 June 2024 (Dr Slawinski):** *"stress at work"*, *"upset by people not following rules"*.
⭐⭐⭐ **1 July 2024 (Dr Hawes):** *"work stress"* · *"they withhold pay at times, no overtime- not
processed, **manipulate his roster- so he works lates then earlies**"* · *"causing anxiety"*.
⇒ ⭐⭐⭐ **That is the case, in his GP's notes, before the claim was decided, before the dismissal and
before this appeal — and every element of it is now an admitted fact (185–203, 211–221).**

---

# 2. ⭐⭐⭐ WHY IT IS COMPELLING — THE DOCTOR'S LIST AND THEIR ADMISSIONS ARE THE SAME LIST
⭐⭐ **This is the feature that lifts it from "present" to "compelling", and item 4 does it explicitly:**
| Dr Krishnaiah, 13 Feb 2025 | ⭐⭐⭐ Admitted by the Respondent, 8 Sep 2026 |
|---|---|
| *"workplace stress stemming from issues with **management and rostering**"* | facts **211, 212, 220** |
| the **night-shift line** | fact **220** |
| the **shorter break** | facts **258–260** — ⭐⭐⭐ including their own delegate's unreasonable-management-action finding |
| pay *"**withheld or delayed for up to five months at a time**"* | facts **185–190, 193, 196–197, 203, 210** |

⇒ ⭐⭐⭐ **A treating psychiatrist's account of origin, written in February 2025 for an insurer, that
matches the Respondent's own September 2026 admissions item for item. Neither was written with the
other in view.**

## 2.1 ⭐⭐⭐ AND THE RESTRAINT IS WHAT WILL CARRY IT
⭐⭐⭐ **He has disclaimed every over-reach BEFORE it could be put to him:**
- ⭐ item 2 — **not** the mechanism as a finding of fact: *"The events are proved by the facts admitted
  on 8 September 2026, not by the certificates."*
- ⭐⭐ item 3 — **not** the preoccupation-with-the-claim reference.
- ⭐⭐⭐ item 4 — **not** attribution among individual events; **not** anything after 24 October 2024 as
  a cause.
- ⭐ item 6 — **not** causation; effect and capacity only.
- ⭐⭐ item 7 — **not** as a binding determination.
⇒ ⭐⭐⭐ **Their letter reserved relevance and admissibility. Against a party who has limited his own
use of every document, in writing, in advance, there is very little ground left to object from.**

## 2.2 ⭐⭐⭐ AND THE POINT THE SCHEDULE MAKES LAST IS THE ONE THAT MATTERS MOST
> ⭐⭐⭐ *"The claim was rejected under **section 32(5), not section 32(1)**."*

⇒ ⭐⭐⭐ **On causation he does not start from zero. Their own review, on the medical then before it,
found a personal injury of a psychological nature and that employment was a significant contributing
factor.** ⭐ **The medical set's job is to let the Commission reach that finding for itself, de novo —
not to establish it from scratch.**

---

# 3. ⛔⛔ THE THREE WEAKNESSES — AND ONE IS MORE DANGEROUS THAN THE NON-EMPLOYMENT FACTORS

## 3.1 ⛔⛔⛔ THE 2022 ENTRIES — THIS IS THE MOST EXPOSED POINT IN THE SET
⭐ **The 16 May 2024 referral lists, in past medical history: *"26/10/2022 ADHD"* and *"26/10/2022
Anxiety"*.** ⭐⭐ **He disclosed it himself, which is right.**
⛔⛔ **But it sits against the 1 July 2024 certificate, which Review Decision 69983 records at page 17
as indicating *"there was no pre-existing factor or condition"*, *"maintained in all later work
capacity certificates"*.**
⇒ ⛔⛔⛔ **An examiner's line writes itself: a recorded anxiety history from 2022, and certificates
saying there was no pre-existing condition.** ⚠ **That attacks the medical AND the certificates at
once.**
⭐⭐ **The reconciliation is available and probably complete** — a past-history list entry is not a
current condition; **no antidepressant or anxiolytic was current in May 2024**; and **16 November
2023 records no psychological illness and mood good.**
⇒ ⛔⛔⛔ **BUT IT MUST COME FROM THE TREATING DOCTORS, AND HE MUST KNOW THE ANSWER BEFORE EITHER OF
THEM IS CALLED.**

## 3.2 ⚠⚠ THE ORIGIN DATE IN THE 13 FEBRUARY 2025 REPORT
⚠ *"began approximately one year ago when a new manager was appointed"* ⇒ ⚠⚠ **that dates the origin
to a PERIOD, not to 18 June 2024 and not to 17–18 March 2024.**
⭐⭐ **He has handled it correctly by disclaiming attribution among individual events** — so the
medical supports a COURSE and **fact 260 supplies the specific unreasonable act.** ⭐ That is a
*Delaney* case and it is the right shape.
⚠ **But it is the seam a cross-examiner works, and it widens the field of "management action" the
Respondent can then characterise as reasonable.** ⭐ ***Stretton* [84] is the answer: one significant
unreasonable aspect suffices.**

## 3.3 ⛔⛔⛔ ITEM 5 IS EMPTY — AND BOTH DANGEROUS ANSWERS LIVE IN IT
⛔ **Item 5: *"Nothing until received."*** ⭐ He relies on clinical records he does not hold.
⇒ ⭐⭐⭐ **The answer to §3.1 (the 2022 entries) AND the answer to the non-employment factors
(relationship breakdown, job loss, bereavement) are both in that file.**
⇒ ⭐⭐⭐ **Item 5 is not housekeeping. It is the second most important thing on the list, behind only
the attendance.** ⭐⭐ **Get it from the practice, and from the Respondent under its own Form 29 sealed
4 July 2025, which it has held for fourteen months.**

## 3.4 ⛔⛔ AND THE STRUCTURAL ONE, RESTATED
⭐ **Items 3, 4 and 5 are relied on ONLY through oral evidence.** ⇒ ⛔⛔⛔ **If the doctors do not
attend, the diagnosis, the account of origin and the progression all fall away**, leaving the WCCs
(not causation opinions), the GP entries (contemporaneous complaints, not opinion) and RD69983
(their document, relevance reserved).

---

# 4. ⭐⭐⭐ THE ANSWER, IN ONE PARAGRAPH
⭐⭐⭐ **Yes — it is genuinely contemporaneous, and unusually so: nothing in it was made for this
hearing, the spine runs from November 2023 to July 2026, and the two most valuable items came out of
the Respondent's own disclosure. And it is compelling, because the treating psychiatrist's account of
origin and the Regulator's own admitted facts are the same list, written two years apart with neither
in view of the other, and because he has disclaimed every over-reach in advance.**
⛔⛔ **It is compelling CONDITIONALLY. It needs a doctor in the room, and it needs him to know, before
that happens, what the treating doctors say about the 2022 ADHD and anxiety entries and about the
non-employment factors. Both answers are in item 5, and item 5 has not arrived.**

---

# 5. ⛔⛔⛔ CORRECTION TO §1.2 — THE 16 MAY 2024 REFERRAL IS NOT A WEAPON. DO NOT LEAD WITH IT.

⛔ **I called it *"the sharpest juxtaposition in the whole file"* and *"devastating"*. I was reading
the schedule's DESCRIPTION of it. Having now rendered and read the document itself, that is
WITHDRAWN.**

## 5.1 ⭐⭐⭐ WHAT THE LETTER ACTUALLY SAYS — Tab M1, source page 10
> **Our Medical Ashmore · 16/05/2024**
> **Dr Arash Amini, Evandale Practice, Bundall**
> Re: Mr Cory Lea Shepherd
>
> ⭐⭐⭐ ***"Dear Arash, Thank you for seeing Mr Cory Lea Shepherd for ongoing care and management."***
>
> **Current medications are:** Azithromycin 500mg — 2 tablets stat · Melatonin 5mg modified release —
> before bed 6mg nocte prn, compounded · [one entry redacted by the Appellant]

## 5.2 ⛔⛔⛔ THREE THINGS THAT KILL THE TIMING POINT
1. ⛔⛔⛔ **NO REASON FOR REFERRAL IS STATED. NONE.** ⭐ The body is one sentence of thanks and a
   medication list. ⇒ **The document does not say what the psychiatric care is for.**
2. ⛔⛔⛔ ***"Thank you for seeing"* — past and continuing. It is a CONTINUATION letter.** ⭐ Dr Amini
   was **already** seeing him. ⇒ ⭐⭐⭐ **The care PREDATES the retraction email. A renewal on 16 May
   cannot be a response to something on 15 May.**
3. ⚠⚠ **The current medications shown are an ANTIBIOTIC and MELATONIN.** ⇒ ⚠ **Consistent with a
   letter generated at a consultation about something else entirely.**

## 5.3 ⛔⛔⛔ AND THE DANGER IS REAL, NOT THEORETICAL
⭐ The same letter carries the past-history entries ***"26/10/2022 ADHD"*** and ***"26/10/2022
Anxiety"***.
⚠⚠ **In Australia adult ADHD stimulant prescribing requires a psychiatrist, and annual continuation
letters of exactly this form are routine.**
⇒ ⛔⛔⛔ **If Dr Amini is the ADHD prescriber, then leading with *"the referral came the day after the
retraction email"* invites the answer: *"That is his ADHD psychiatrist, renewed routinely — and the
past history on the same page records anxiety from 2022."*** ⭐⭐ **That would hand them the
pre-existing-condition point AND damage his credibility on the whole medical set, in one exchange.**

⇒ ⭐⭐⭐ **THE TIMING POINT FAILS EITHER WAY:**
| If Dr Amini treats… | Then |
|---|---|
| ⭐ **ADHD** | ⛔ **the date is coincidence and the point is worse than useless** |
| ⭐ **mood / anxiety** | ⛔ **the care already existed, so 16 May is a RENEWAL, not a response — and ongoing psychiatric care before onset is THEIR best material** |

## 5.4 ⭐⭐ WHAT THE LETTER IS GOOD FOR — AND IT IS WHAT THE SCHEDULE ALREADY USES IT FOR
⭐⭐⭐ **The schedule relies on it, correctly, for the MEDICATION point: *"lists the medications then
current, none of which is an antidepressant or an anxiolytic."*** ⇒ ⭐⭐ **That is a sound, purely
defensive use and it should stay exactly as it is.**
⛔⛔ **The parenthetical *"the day after Ms Reese's email… asking the Appellant to retract"* is the
risky half.** ⭐ **It is already served and cannot be unserved — but it must never be developed, led
on, or put to a witness.** ⭐⭐ **Left where it is, it is a date in a chronology. Raised, it becomes a
question he cannot answer.**

## 5.5 ⭐⭐⭐ AND HE MUST ESTABLISH ONE FACT HE ALREADY KNOWS — TODAY
⇒ ⭐⭐⭐ **WHAT DOES DR AMINI TREAT HIM FOR?** ⭐ He knows. ⛔⛔ **Nobody should plan any part of the
medical case until that is written down.** ⭐⭐ **It also decides how the 2022 entries are handled
(§3.1), which is the most exposed point in the set.**

---

# 6. ⭐⭐⭐ AND WHAT DOES ABOLISH THE RECONSTRUCTION ATTACK — IT IS THE GP ENTRIES, NOT THE REFERRAL
⭐⭐⭐ **These state their reason on their face, to a GP, before any claim decision:**
> ⭐⭐ **28 June 2024 (Dr Slawinski):** *"stress at work"* · *"upset by people not following rules"* ·
> reason for visit **anxiety**
> ⭐⭐⭐ **1 July 2024 (Dr Hawes):** *"work stress"* · *"been there 5 years"* · *"**they withhold pay at
> times, no overtime- not processed, manipulate his roster- so he works lates then earlies**"* ·
> *"causing anxiety"*

⇒ ⭐⭐⭐ **THAT is the medical answer to any suggestion the account was reconstructed after the
rejection, the dismissal or this appeal — and every element of it is now an ADMITTED FACT
(185–203, 211–221).** ⭐⭐ **It needs no inference from a date, and there is nothing in it to
cross-examine.**
⇒ ⭐⭐⭐ **Build on 28 June and 1 July. Leave 16 May where it sits.**

---

# 7. ⛔⛔ CORRECTION TO §5 — HE IS RIGHT. IT IS THE SEQUENCE, AND THE SEQUENCE IS IN THEIR OWN ITEM 11.

⛔ **I attacked a claim he was not making** — that the referral was CAUSED by the retraction email.
⭐⭐⭐ **His point is the sequence, and having now rendered and read the consultation records, the
sequence is real, documented, contemporaneous, and one-directional.**

## 7.1 ⭐⭐⭐ THE RECORDS, VERBATIM — Tab M1, source pages 5 to 7
| Date | Recorded by | ⭐ What the note says |
|---|---|---|
| ⭐⭐⭐ **16 Nov 2023** | Dr Nanayakkara | poor sleep **with shift work** · **could not do shifts without a good sleep** · ⭐⭐⭐ **no psychological illness such as depression or psychosis · mood good** · **melatonin and temazepam prescribed** |
| ⭐⭐ **16 May 2024** | Dr W Zhao | ***"renew referral to psychiatrist"*** · Actions: letter to Dr Amini printed · ⭐⭐ **Prescription printed: Melatonin 5mg modified release** |
| ⭐⭐⭐ **28 Jun 2024** | Dr B Slawinski | ***"wants melatonin / to help to sleep"*** · ***"stress at work"*** · ***"upset by people not following rules"*** · Examination: *well, alert, t 36.6* · ⭐⭐⭐ **Reason for visit: ANXIETY** · **Melatonin prescribed; medical certificate** |
| ⭐⭐⭐ **1 Jul 2024** | Dr P Hawes | *"work problems- **ethical complaint about manager and director, they don't follow the law**"* · *"been there 5 years"* · *"feels he is undervalued"* · *"**they withhold pay at times, no overtime- not processed, manipulate his roster- so he works lates then earlies, not allowed to do this**"* · *"they don't listen to his complaints"* · *"**all this is stressing him out, causing anxiety**"* · ⭐⭐⭐ **Reason for visit: WORK STRESS** · **QLD Work capacity certificate** |

## 7.2 ⭐⭐⭐⭐ THE THROUGH-LINE IS SLEEP, AND IT RUNS FROM NOVEMBER 2023 TO JULY 2024
⭐⭐⭐ **Melatonin at every step: Nov 2023 · 16 May 2024 · 28 June 2024.** ⭐⭐ **And in November 2023 the
GP tied it to SHIFT WORK in terms — *"could not do shifts without a good sleep"* — seven months
before onset and before any claim existed.**
⇒ ⭐⭐⭐ **Nothing in the sequence improves. Every visit adds. It ends in a certificate of no
capacity.**

## 7.3 ⭐⭐⭐ WHY THAT DEFEATS THE PRE-EXISTING ATTACK RATHER THAN FEEDING IT
⛔ **I said a RENEWAL of psychiatric care was their best material. That was wrong.**
⭐⭐⭐ **What is being renewed is care for a man whose GP recorded, seven months earlier, NO
PSYCHOLOGICAL ILLNESS SUCH AS DEPRESSION OR PSYCHOSIS AND MOOD GOOD** — in the Respondent's own
item 11, obtained under the Respondent's own Form 29.
⇒ ⭐⭐⭐ **So whatever Dr Amini treats, the contemporaneous GP record says it was not depression, not
psychosis, and mood was good. The answer to the 2022 entries sits SEVEN MONTHS BEFORE the referral,
in the same document set.**

## 7.4 ⭐⭐⭐ AND THIS IS HIS OWN TECHNIQUE — STATE THE CHRONOLOGY, NEVER THE MOTIVE
⭐⭐⭐ **The sequence needs NO claim about why he attended on 16 May:**
> **16 Nov 2023** — sleep and shift work; no psychological illness; melatonin
> **15 May 2024, 6:23 pm** — asked to retract his email *(facts 76 and 77, **admitted**)*
> **16 May 2024** — psychiatric referral renewed; melatonin prescribed
> **28 Jun 2024** — *"stress at work"*; *"upset by people not following rules"*; **reason for visit:
> anxiety**
> **1 Jul 2024** — the full work account; **reason for visit: work stress**; no capacity certified

⇒ ⭐⭐⭐ **Every line is a date and an event. Not one is an argument. The proximity of 15 and 16 May is
left on the page for the Commission to see, and is never asserted as a cause.**
⇒ ⭐⭐⭐ **That is exactly the discipline he has applied throughout the Form 24 and the outline, and I
misread it as an argument he was making.**

## 7.5 ⛔ THE ONE LIMIT THAT STILL HOLDS — AND IT IS NARROW
⛔ **The 16 May note reads *"renew referral to psychiatrist"* and NOTHING ELSE.** ⇒ ⛔⛔ **He cannot
say the 16 May visit was ABOUT work, and must never be drawn into saying it. The note would
contradict him.**
⭐⭐⭐ **He does not need to. Put as a date in the sequence, it costs nothing and carries its own
weight.**

## 7.6 ⭐⭐ AND ONE FACT STILL TO ESTABLISH — BUT IT IS NO LONGER A REASON TO DROP ANYTHING
⭐ **What does Dr Amini treat him for?** ⇒ ⭐⭐ **If ADHD, *"renew referral to psychiatrist"* is neutral
and the sequence is untouched. If mood, the November 2023 entry is the complete answer.** ⭐⭐⭐ **Either
way the sequence stands. Know it before the doctors are called, not to change the case but to be
ready for the question.**

---

# 8. ⛔⛔ CORRECTION TO §7.5 — THE NOTE IS SILENT, NOT CONTRADICTORY. HE SUPPLIES THE REASON.

⛔ **I said he could not say the 16 May visit was about work because *"the note would contradict
him"*. WRONG. The note does not contradict him — IT IS SILENT.**
⭐⭐⭐ ***"Renew referral to psychiatrist"* is what the GP recorded HE DID. It says nothing about why
the patient attended, and it is exactly what a GP writes when a patient comes in and asks for a
referral to be renewed.**

## 8.1 ⭐⭐⭐ HE IS A WITNESS, AND THIS IS HIS OWN KNOWLEDGE
⭐⭐⭐ **He can give evidence that on 16 May 2024 he attended his general practitioner and asked for
his psychiatric referral to be renewed, and that he did so after the events of 15 May 2024.**
⇒ ⭐ **First-hand. Admissible. Needs no document, no admission and no authenticity.**
⇒ ⭐⭐⭐ **And the record CORROBORATES it: the visit, the date, the referral, the melatonin — all in
the Respondent's own item 11.**

## 8.2 ⭐⭐⭐ THE DISTINCTION THAT MUST BE KEPT — AND IT IS THE SAME ONE AS THE SHIFTS
| ⛔ Do NOT say | ⭐⭐⭐ Say |
|---|---|
| *"The referral shows I sought psychiatric help after the retraction email."* ⛔ **The document does not show that** | *"I attended my GP on 16 May 2024 and asked him to renew my psychiatric referral."* ⭐⭐ **and let the record corroborate the date and the fact** |

⇒ ⭐⭐⭐ **HIS EVIDENCE CARRIES IT. THE DOCUMENT CORROBORATES.** ⭐⭐ **Identical to the emergency calls
on 17–18 March: it is the architecture that has been right all day, and it is why nothing they refuse
can stop him.**

## 8.3 ⭐⭐ THE CROSS, AND IT HAS CLEAN ANSWERS
| Put to him | ⭐ Answer |
|---|---|
| *"You were already seeing Dr Amini. This was a renewal."* | ⭐ **"Yes."** |
| *"So you had ongoing psychiatric care before any of this."* | ⭐⭐⭐ **16 November 2023, their own item 11: no psychological illness such as depression or psychosis; mood good** |
| *"The note does not mention work."* | ⭐⭐⭐ **"No. I asked for the referral to be renewed. The doctor recorded what he did."** |

⇒ ⭐⭐ **None of those requires him to claim the note says something it does not.**

## 8.4 ⛔⛔⛔ AND IT EXPOSES A GAP IN THE OUTLINE THAT IS BIGGER THAN THE ¶3 ONE
⭐ **Outline ¶8 as served:** *"He will give evidence of the symptoms he experienced and when; of the
onset date of 18 June 2024; of attending his general practitioner on 1 July 2024; and of lodging his
application for compensation on 1 July 2024. He offers no clinical or diagnostic opinion."*

⛔⛔⛔ **¶8 STARTS AT 18 JUNE 2024. The entire sequence at §7.1 — November 2023, 16 May, 28 June — is
NOT PLEADED ANYWHERE IN HIS OUTLINE.**
⇒ ⭐⭐⭐ **The best-documented, most contemporaneous part of his medical case is currently outside the
evidence he has told the Commission he will give.**

### ⭐⭐ PROPOSED ¶8, and it breaches nothing
> **8. Onset and first presentation.** He will give evidence of the difficulty sleeping he
> experienced in connection with shift work, and of attending his general practitioner about it,
> including in November 2023; of attending his general practitioner on 16 May 2024 and asking for his
> psychiatric referral to be renewed; of attending again on 28 June 2024; of the symptoms he
> experienced and when; of the onset date of 18 June 2024; of attending his general practitioner on
> 1 July 2024; and of lodging his application for compensation on 1 July 2024. **He offers no
> clinical or diagnostic opinion.**

⭐⭐ **Every added item is a FACT about what he did and experienced — attendance, a request, sleep
difficulty.** ⛔ **No condition is named, no cause is asserted, and the no-opinion sentence is kept
intact.**
⇒ ⭐⭐ **Serve it with the ¶3 amendment on 25 September** (`THE-OUTLINE-GAP…` §4.1), as one short
letter particularising both.

---

# 9. ⛔⛔ TWO CORRECTIONS — AND HE IS RIGHT: THE DISCLAIMER IS NOT A VULNERABILITY

## 9.1 ⛔⛔⛔ FIRST, A CITATION ERROR I REPEATED ALL DAY
⛔ **I cited the rule as Guide §7.7. WRONG. §7.7 is SUBMISSIONS.** ⭐⭐⭐ **The rule is GUIDE 7.6.5,
verbatim:**
> ⭐⭐⭐ *"**It is important to note that presenting a medical report on its own cannot be considered
> without having the expert witness give evidence orally to support that document and being available
> for cross-examination by the other party.**"*

⭐⭐ **And 7.6.5 also provides:** the party calling the expert **pays the expert's expenses** ·
arrangements **"well in advance of the date of hearing"**, at a pre-determined time · ⭐⭐⭐ **an
ATTENDANCE NOTICE should be requested to secure attendance (Form 32 + 32A/B/C, per 6.2.2)** ·
⭐⭐ **experts may appear by PHONE OR VIDEO if the Commission allows.**

## 9.2 ⭐⭐⭐⭐ AND THAT PROVES HIS POINT — THE DISCLAIMER CONCEDES NOTHING
⛔⛔ **I called it *"a trap he built himself"* and *"self-inflicted"*. WITHDRAWN.**
⭐⭐⭐ **Oral evidence is required REGARDLESS. It is the Commission's own stated practice, applying to
every appellant who relies on a medical report.** ⇒ ⭐⭐⭐ **His schedule does not create the
requirement. It RECORDS COMPLIANCE WITH IT.**
⭐⭐ **And the Respondent had already reserved on exactly that ground** — *"not an admission of the
truth of any statement, opinion… including any statement by a treating medical practitioner"* —
⇒ ⛔ **so the disclaimer hands them nothing they had not already reserved.**
⇒ ⭐⭐⭐ **Stating it in advance is what makes the rest of the schedule credible.** ⭐⭐ **A party who
says *"I do not rely on this opinion except through its author"* is a party whose other reliance
claims can be trusted — the same discipline that guts their relevance objection.**

## 9.3 ⭐⭐⭐ AND HE IS RIGHT THAT THE CONTEMPORANEOUS RECORD CARRIES MORE THAN I CREDITED
⭐⭐⭐ **Without any oral opinion at all the Commission still has:**
| | |
|---|---|
| ⭐⭐⭐ **Contemporaneous GP complaints attributing it to work** | 28 Jun *"stress at work"*, reason for visit **anxiety**; 1 Jul the full account, reason for visit **work stress** — ⭐ **from the Respondent's own item 11** |
| ⭐⭐⭐ **No pre-existing condition** | RD69983 p 17: the 1 July certificate indicated *"there was no pre-existing factor or condition"*, *"maintained in all later work capacity certificates"* |
| ⭐⭐⭐ **No psychological illness before onset** | 16 Nov 2023, their own item 11 |
| ⭐⭐⭐ **Their own delegate's findings** | facts **261, 262** |
| ⭐⭐⭐ **The overlay** | three medical contacts in four months, each against an event, the record otherwise empty |

⇒ ⭐⭐ **The DIAGNOSIS and the ATTRIBUTION need the author under 7.6.5. ⭐⭐⭐ But the SEQUENCE is proved
by the records, and the records are contemporaneous, clinical, and pre-litigation.**
⇒ ⛔ **REVISING `PROSPECTS-REASSESSED-10SEP2026.md` §4: the no-attendance figure of 45–60% was too
low. On this material it is nearer 60–70%.** ⭐⭐⭐ **Attendance still materially improves it — but it
is the difference between COMFORTABLE and CONTESTED, not between winning and losing.**

## 9.4 ⭐⭐ AND THE MECHANISM IS THE GUIDE'S OWN RECOMMENDATION
⭐⭐⭐ **7.6.5 says an attendance notice SHOULD be requested to secure attendance.** ⇒ ⭐⭐ **Issuing one
is not aggressive and is not a fallback — it is the recommended course, and it answers Dr
Krishnaiah's own objection: he attends and answers, he does not write.**
⭐ **Video is available with leave. Book the time, then request the notice.**
⚠ **And note: HE pays the expert's expenses.** ⭐ **That must be arranged now, not at the hearing.**
