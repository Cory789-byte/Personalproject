...
def tab_sheet(t):
    ...
    s=[P(HEADER[0],HD),P(HEADER[1],HD2),P(f"TAB M{t['n']}",TAB),P(t['title'],TT),
       ...
       Spacer(1,4*mm),P({9:"This tab is added so that the Respondent's own position on the documents at Tabs M1, M2, M4 and M8 sits with them.",
         5:"This tab is added so that the question, the answer and the notice the question came from can be read together with the report at Tab M4, which precedes it. Schedule row 4 states what the report is, and is not, relied upon for."}.get(t['n'], "Schedule row "+str({6:5,7:6,8:7}.get(t['n'],t['n']))+" states what this document is, and is not, relied upon for."),B)]
