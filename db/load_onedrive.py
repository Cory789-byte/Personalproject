...
Usage:
    ...
    # uses your Windows OneDrive sign-in automatically — Windows sets %OneDrive%
    # the moment you log in to OneDrive desktop:
    python db/load_onedrive.py

    # override the auto-discovered path:
    python db/load_onedrive.py --root "C:\\Users\\you\\OneDrive\\Southport"
...
from _pc_user import onedrive_root, pc_user
...
def upsert(cur, matter_id: int, provider: str, root: Path, path: Path,
           st: os.stat_result, do_hash: bool, skipped_hash: bool,
    ...
    cur.execute(
        ...
        (
            ...
            sha, datetime.fromtimestamp(st.st_mtime), ingested_by,
        ),
    )
...
def main() -> int:
    ...
    p.add_argument("--root", default=None,
                   help="override the auto-discovered OneDrive root")
    ...
    root = Path(args.root) if args.root else onedrive_root()
    if root is None:
        sys.stderr.write(
            "could not auto-discover OneDrive root and --root not given.\n"
            "Sign in to OneDrive desktop (sets %OneDrive%) or pass --root explicitly.\n"
        )
        return 2
    if not root.is_dir():
    ...
    print(f"cloud files seen={seen} hashed={hashed} (matter {args.matter_ref}, "
          f"provider {args.provider}, root {root}, ingested_by {user})")
    return 0
